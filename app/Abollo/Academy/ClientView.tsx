      ...
      <div className="mx-auto max-w-6xl space-y-10 px-4 py-12 text-white">
        ...
        <section className="space-y-6">
          {htmlCssPath.sections.map((section) => (
            <div
              key={section.id}
              className="space-y-4 rounded-3xl border border-white/5 bg-slate-900/60 p-6"
            >
              <div>
        ...
        <section className="space-y-4 rounded-3xl border border-white/5 bg-slate-900/70 p-6">
          <div className="flex flex-col gap-2">
            ...
            <h2 className="text-3xl font-semibold text-white">
              CodeBin — build beside the lesson
            </h2>
            <p className="text-sm text-slate-300">
              Experiment with the current concept, save snapshots to your
              browser, import previous zips, or export a bundle to share. The
              environment mirrors Sandpack (CodeSandbox) so what you see here
              matches production.
            </p>
          </div>
          <LivePlayground
